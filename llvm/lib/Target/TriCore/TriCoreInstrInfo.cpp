...
void TriCoreInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
	...
	bool DataRegsDest = TriCore::DataRegsRegClass.contains(DestReg.id());
	bool DataRegsSrc = TriCore::DataRegsRegClass.contains(SrcReg.id());
	...
	bool AddrDest = TriCore::AddrRegsRegClass.contains(DestReg.id());
	bool AddrSrc = TriCore::AddrRegsRegClass.contains(SrcReg.id());
