...
createTriCoreMCSubtargetInfo(const Triple &TT, StringRef CPU, StringRef FS) {
  return createTriCoreMCSubtargetInfoImpl(TT, CPU, CPU, FS);
}
...
static MCInstPrinter *createTriCoreMCInstPrinter(const Triple &TT,
...
extern "C" void LLVMInitializeTriCoreTargetMC() {
  ...
  Target &TheTriCoreTarget = getTheTriCoreTarget();

  // Register the MC asm info.
  RegisterMCAsmInfoFn X(TheTriCoreTarget, createTriCoreMCAsmInfo);

  // Register the MC instruction info.
  TargetRegistry::RegisterMCInstrInfo(TheTriCoreTarget,
                                      createTriCoreMCInstrInfo);

  // Register the MC register info.
  TargetRegistry::RegisterMCRegInfo(TheTriCoreTarget,
  ...
  TargetRegistry::RegisterMCInstPrinter(TheTriCoreTarget,
                                        createTriCoreMCInstPrinter);

  // Register the ASM Backend.
  TargetRegistry::RegisterMCAsmBackend(TheTriCoreTarget,
                                       createTriCoreAsmBackend);

  // Register the MCCodeEmitter
  TargetRegistry::RegisterMCCodeEmitter(TheTriCoreTarget,
                                        createTriCoreMCCodeEmitter);
}
