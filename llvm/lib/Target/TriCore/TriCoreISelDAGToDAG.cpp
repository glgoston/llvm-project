...
void TriCoreDAGToDAGISel::Select(SDNode *N) {
  ...
  case TriCoreISD::MADD: {
    ...
    if (Acc && X && Y) {
      CurDAG->SelectNodeTo(N, TriCore::MADDrrr, MVT::i32, Acc, X, Y);
      return;
    }
    break;
  }
  case TriCoreISD::MSUB: {
    ...
    SDValue Y   = getI32BitPatternOperand(N->getOperand(2));
    if (Acc && X && Y) {
      CurDAG->SelectNodeTo(N, TriCore::MSUBrrr, MVT::i32, Acc, X, Y);
      return;
    }
    break;
  }
  ...
  case TriCoreISD::MADDSU: {
    ...
    SDValue Y   = getI32BitPatternOperand(N->getOperand(2));
    if (Acc && X && Y) {
      CurDAG->SelectNodeTo(N, TriCore::MADDSUrrr, MVT::i32, Acc, X, Y);
      return;
    }
    break;
  }
  case TriCoreISD::MSUBSU: {
    ...
    SDValue Y   = getI32BitPatternOperand(N->getOperand(2));
    if (Acc && X && Y) {
      CurDAG->SelectNodeTo(N, TriCore::MSUBSUrrr, MVT::i32, Acc, X, Y);
      return;
    }
    break;
  }
