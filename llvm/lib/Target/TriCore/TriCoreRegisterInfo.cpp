...
bool TriCoreRegisterInfo::trackLivenessAfterRegAlloc(
    const MachineFunction &MF) const {
  return true;
}

bool TriCoreRegisterInfo::useFPForScavengingIndex(
    const MachineFunction &MF) const {
  return false;
}

// const TargetRegisterClass *
// TriCoreRegisterInfo::getPointerRegClass(const MachineFunction &MF, unsigned
// Kind)
//                                                                         const
//                                                                         {
...
bool TriCoreRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  if (MI.getOpcode() == TriCore::ADDrc) {
    ...
    Offset = -Offset;
    //		outs().changeColor(raw_ostream::GREEN, 1) << "Offset: " <<
    //Offset
    //<<
